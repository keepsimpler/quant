...
inSize = outSize = 1
resSize = 1000 # number of nodes in the reservior
a = 0.3 # leaking rate
...
x = rep(0, resSize) # initial state variables
for (t in 1:trainLen){
  u = data[t]
...
X_T = t(X)
Wout = Yt %*% X_T %*% 
  solve( X %*% X_T + reg * diag(1 + inSize + resSize) )
...
errorLen = 500
mse = ( sum( (data[(trainLen+2):(trainLen+errorLen+1)] -
                Y[1,1:errorLen])^2 ) / errorLen )
print( paste( 'MSE = ', mse ) )
...
#' @title Echo State Network pridicte price
#' @param prices, a <xts> object
#' @param coeffs, a list of ESN parameters
#' @return ret, predicted price
#' 